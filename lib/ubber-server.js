...
module.exports = {
  start: function(options, callback){
    ...
    if(env === 'dev'){
      ...
      if(options.verbose) console.log('Compile templates from ', Path.resolve(path, 'pages'), 'to', Path.resolve(path, 'pages.js'));
...
function compileTemplates(path, cb){
  jst.underscore(Path.resolve(path, 'pages'), {verbose: true}, function(err, compiledTemplates){
    if(err) return console.error(err);
    fs.writeFile(Path.resolve(path, 'pages.js'), compiledTemplates.join('\n'),'utf-8', cb);
  });
}
